grades = {"A1": 100, "A2": 95, "A3": 90, "B1": 85, "B2": 80, "B3": 75, "C1": 70, "C2": 65, "C3": 60, "D1": 55,
          "D2": 50, "D3": 45, "E1": 40, "E2": 35, "E3": 30, "F1": 25, "F2": 20, "F3": 15, "G1": 10, "G2": 5, "G3": 0}
...
def get_max_grade(*students):
    numerical_grade = [grades[students[i].get_grade()]
                       for i in range(len(students))]
    max_grade =  max(numerical_grade)
    student_with_max_grade = [students[i].name for i in range(
        len(students)) if grades[students[i].get_grade()] == max_grade]
    return str(student_with_max_grade)


max_grade = get_max_grade(bob, jeffery, tim)
print(max_grade.strip("[]"))